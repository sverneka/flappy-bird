...
def trainNetwork(models,args):
    ...
    while (True):
        ...
        printInfo(t, state, action_index, r_t, Q_sa, Q_sa+np.sqrt(2*np.log(t)/(Nt)), loss, chosen)
...
def printInfo(t, state, action_index, r_t, Q_sa, modified_Q_sa, loss, chosen):
    print("TIMESTEP", t, "/ STATE", state, \
          "/ ACTION", action_index, "/ REWARD", r_t, \
          "/ Q_MAX " , np.max(Q_sa), "/ MODIFIED ", np.max(modified_Q_sa), \
          "/ Loss ", loss, "/ CHOSEN HEAD", chosen)
